                ...
                <form id="updateAccountForm" onsubmit="submitUpdate(event)" enctype="multipart/form-data">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px;">
                        <div class="form-group">
                            <label style="font-weight: 800; font-size: 0.75rem; color: var(--secondary); display: block; margin-bottom: 8px; text-transform: uppercase;">Full Name</label>
                            <input type="text" name="name" value="{{ $user->name }}" class="form-control" style="border-radius: 12px; height: 50px; border: 2px solid var(--border-color); font-weight: 600; background: var(--bg-body); color: var(--text-main);">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="email" name="email" value="{{ $user->email }}" class="form-control" style="border-radius: 12px; height: 50px; border: 2px solid var(--border-color); font-weight: 600; background: var(--bg-body); color: var(--text-main);">
                        </div>
                        <div class="form-group">
                            <label style="font-weight: 800; font-size: 0.75rem; color: var(--secondary); display: block; margin-bottom: 8px; text-transform: uppercase;">New Password</label>
                            <input type="password" name="password" placeholder="Leave blank to keep current" class="form-control" style="border-radius: 12px; height: 50px; border: 2px solid var(--border-color); font-weight: 600; background: var(--bg-body); color: var(--text-main);">
                        </div>
                        <div class="form-group">
                            <label style="font-weight: 800; font-size: 0.75rem; color: var(--secondary); display: block; margin-bottom: 8px; text-transform: uppercase;">Account Role</label>
                            @if(in_array(auth()->user()->role, ['admin', 'super_admin']))
                            <select name="role" class="form-control" style="border-radius: 12px; height: 50px; border: 2px solid var(--border-color); font-weight: 600; background: var(--bg-body); color: var(--text-main);">
                            ...
                            @else
                            <div class="form-control" style="border-radius: 12px; height: 50px; border: 2px solid var(--border-color); font-weight: 700; background: var(--bg-body); display: flex; align-items: center; color: var(--secondary); cursor: not-allowed; text-transform: capitalize;">
                        ...
                        <div class="form-group" style="grid-column: span 2; position: relative;">
                            ...
                            @if(in_array(auth()->user()->role, ['admin', 'super_admin']))
                            <div class="custom-dropdown" style="border: 2px solid var(--border-color); border-radius: 12px; background: var(--bg-body); cursor: pointer;">
                                <div id="dropdownHeaderShow" class="dropdown-header" onclick="toggleDropdown('permissionsDropdownShow')" style="height: 50px; display: flex; align-items: center; justify-content: space-between; padding: 0 15px; font-weight: 600; color: {{ count($userAccessList) > 0 ? '#6366f1' : 'var(--secondary)' }}; font-size: 0.9rem;">
                                    <span style="{{ count($userAccessList) > 0 ? 'font-weight: 800;' : 'font-weight: 600;' }}">{{ count($userAccessList) > 0 ? count($userAccessList) . ' role(s) selected' : 'Select Permissions...' }}</span>
                                    <i class="fas fa-chevron-down"></i>
                                </div>
                                <div class="dropdown-body" id="permissionsDropdownShow" style="display: none; position: absolute; top: calc(100% + 5px); left: 0; right: 0; background: var(--bg-card); border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 2px solid var(--border-color); z-index: 10;">
                                    <div style="padding: 10px; border-bottom: 1px solid var(--border-color);">
                                        <input type="text" placeholder="Search position..." class="form-control" onkeyup="filterDropdown(this, 'permissionsListShow')" style="border-radius: 8px; height: 35px; border: 1px solid var(--border-color); font-size: 0.85rem; padding-left: 10px; width: 100%; background: var(--bg-body); color: var(--text-main);">
                                    </div>
                                    <div id="permissionsListShow" style="max-height: 200px; overflow-y: auto; padding: 10px;">
                                        ...
                                        <label class="dropdown-item-label" style="display: flex; align-items: center; gap: 8px; font-size: 0.85rem; font-weight: 600; color: var(--text-main); cursor: pointer; line-height: 1.2; padding: 8px; border-radius: 6px; transition: 0.2s;" onmouseover="this.style.background='var(--hover-color)'" onmouseout="this.style.background='transparent'">
                                            <input type="checkbox" name="access[]" value="{{ $rListItem }}" style="width: 16px; height: 16px; accent-color: #6366f1;" {{ in_array($rListItem, $userAccessList) ? 'checked' : '' }} onchange="updateDropdownText('permissionsListShow', 'dropdownHeaderShow')">
                            ...
                            @else
                            <div style="display: flex; flex-wrap: wrap; gap: 8px; padding: 10px; background: var(--bg-body); border-radius: 12px; border: 2px solid var(--border-color);">
                                @forelse($userAccessList as $accessLevel)
                                    <span style="background: rgba(99, 102, 241, 0.1); color: #6366f1; font-weight: 700; font-size: 0.85rem; padding: 8px 14px; border-radius: 8px; border: 1px solid rgba(99, 102, 241, 0.2);">
                                        {{ trim($accessLevel) }}
                                    </span>
                                @empty
                                    <span style="color: var(--secondary); font-size: 0.85rem; padding: 4px; font-weight: 600;">No permissions assigned.</span>
                                @endforelse
                            </div>
                            @endif
                        </div>
                    ...
                    <div style="margin-top: 30px; display: flex; justify-content: flex-end; gap: 15px;">
                        <button type="button" class="btn btn-secondary" style="border-radius: 12px; font-weight: 800; padding: 12px 30px;">Cancel</button>
                        <button type="submit" class="btn btn-primary" style="background: linear-gradient(135deg, #6366f1 0%, #a855f7 100%); border: none; border-radius: 12px; font-weight: 800; padding: 12px 40px; box-shadow: 0 4px 15px rgba(99, 102, 241, 0.3);">
                            Update Account
                        </button>
                    </div>
                </form>
