...
<div class="animate-fade">
    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 24px;">
        ...
        <div class="card glass" style="text-align: center;">
            <h4 style="font-weight: 800; margin-top: 10px;">{{ $employee->full_name }}</h4>
            ...
            <div style="text-align: left; font-size: 0.85rem;">
                <div style="margin-bottom: 12px;"><i class="fas fa-id-badge" style="width: 20px; color: var(--primary);"></i> <strong>Ref ID: {{ $employee->employee_id }}</strong></div>
                <div style="margin-bottom: 12px;"><i class="fas fa-building" style="width: 20px; color: var(--primary);"></i> {{ $employee->department->name ?? 'No Office/Department' }}</div>
                ...
                <div style="margin-bottom: 12px;"><i class="fas fa-venus-mars" style="width: 20px; color: var(--primary);"></i> {{ $employee->gender ?? 'Not Specified' }}</div>
            </div>

            <div style="display: flex; gap: 10px; margin-top: 20px;">
                <a href="{{ route('employees.edit', $employee) }}" class="btn btn-primary" style="flex: 1;" target="_self"><i class="fas fa-edit"></i> Edit</a>
                <a href="{{ route('leave-cards.show', $employee) }}" class="btn btn-secondary" style="flex: 1;" target="_self"><i class="fas fa-address-card"></i> Ledger</a>
            </div>
        </div>
            ...
            <div class="card glass" style="margin-bottom: 20px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                    <div style="text-align: center; padding: 15px; background: #f0f9ff; border-radius: 12px;">
                        ...
                        <h3 style="font-weight: 800; color: var(--primary);">{{ (float)$currentLeaveCard->vl_balance }}</h3>
                    </div>
                    <div style="text-align: center; padding: 15px; background: #f0fdf4; border-radius: 12px;">
                        ...
                        <h3 style="font-weight: 800; color: var(--success);">{{ (float)$currentLeaveCard->sl_balance }}</h3>
                    </div>
                    <div style="text-align: center; padding: 15px; background: #fffbeb; border-radius: 12px;">
                        ...
                        <h3 style="font-weight: 800; color: var(--warning);">{{ (float)$currentLeaveCard->vl_used }}</h3>
                    </div>
                    <div style="text-align: center; padding: 15px; background: #fef2f2; border-radius: 12px;">
                        ...
                        <h3 style="font-weight: 800; color: var(--danger);">{{ (float)$currentLeaveCard->sl_used }}</h3>
                    </div>
                </div>
            </div>
            @endif
