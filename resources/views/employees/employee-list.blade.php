...
<div class="animate-fade">
    <div class="card glass animate-fade">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
            ...
            <a href="{{ route('employees.create') }}" class="btn btn-primary" target="_self">
                <i class="fas fa-plus"></i> Add New Employee
            </a>
        </div>

        <!-- Filters -->
        <form action="{{ route('employees.index') }}" method="GET" target="_self" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 25px; background: #f8fafc; padding: 20px; border-radius: 16px;">
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <a href="{{ route('employees.index') }}" class="btn btn-secondary" target="_self">Reset</a>
            </div>
        </form>
            ...
            <table class="table" style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="text-align: left; border-bottom: 1px solid #eee; color: var(--secondary);">
                        <th style="padding: 15px;">Reference ID</th>
                        <th style="padding: 15px;">Full Name</th>
                        <th style="padding: 15px;">Office/Department</th>
                ...
                <tbody>
                    @if($employees->count() > 0)
                        ...
                        <tr style="border-bottom: 1px solid #f9f9f9; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                            ...
                            <td style="padding: 15px;">{{ $emp->department?->name ?? 'N/A' }}</td>
                            ...
                            <td style="padding: 15px;">
                                <div style="display: flex; gap: 8px;">
                                    <a href="{{ route('employees.show', $emp) }}" class="btn btn-sm btn-light" title="View" target="_self"><i class="fas fa-eye"></i></a>
                                    <a href="{{ route('employees.edit', $emp) }}" class="btn btn-sm btn-light" title="Edit" target="_self"><i class="fas fa-edit"></i></a>
                                    <a href="{{ route('leave-cards.show', $emp) }}" class="btn btn-sm btn-light" title="Leave Card" target="_self"><i class="fas fa-address-card"></i></a>
                                </div>
                            </td>
                        </tr>
                        @endforeach
